...
mean_0, cov_0 = np.array([0, 0]), np.array([[1, 0], [0, 1]]) # initial latent state parameters
lamda = 0.8

A_ = np.array([[lamda, 0], [0, lamda]])
Q_ = np.eye(2)
C_ = np.array([[1, -0.5]])
R_ = 0.1*np.eye(1)
no_points = 200
...
##plt.subplot(121)
##plt.plot(np.arange(-1, len(mean_post) - 1), mean_post[:, 0], color = 'red')
##plt.plot(np.array(xs)[:, 0], color = 'blue')
##mean = mean_post[:, 0]
##std = cov_post[:, 0, 0]**0.5
##
##plt.fill_between(np.arange(-1, len(mean) - 1), mean + 2*std, mean - 2*std,
##                 facecolor = 'grey', alpha = 0.5, label = 'Confidence')
##
##plt.subplot(122)
##plt.plot(np.arange(-1, len(mean_post) - 1), mean_post[:, 1], color = 'red')
##plt.plot(np.array(xs)[:, 1], color = 'blue')
##mean = mean_post[:, 1]
##std = cov_post[:, 1, 1]**0.5
##
##plt.fill_between(np.arange(-1, len(mean) - 1), mean + 2*std, mean - 2*std,
##                 facecolor = 'grey', alpha = 0.5, label = 'Confidence')
##
...
os.chdir('online_plots/noise_2d')
for i in range(20):
