...
mu_init = np.array([np.array([-0.5, -1.5]), np.array([-0.5, 1]), np.array([1, 0.75])]) # initial means judged by eye
...
os.chdir('clust_convergence_animation_patho')
for i in range(2*max_steps + 1):
    ...
    plt.xlim([-1, 2*max_steps + 2]), plt.ylim([-300, -50])
